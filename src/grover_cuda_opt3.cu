...
int main(int argc, char* argv[]) {
    ...
    for (int s=0; s<nStreams; ++s) {
        int length = N / nStreams;
        int offset = s * length;
        cudaMemcpyAsync(state_d + offset, state_h + offset, length, cudaMemcpyHostToDevice, streams[s]);
        cudaMemcpyAsync(new_state_d + offset, new_state_h + offset, length, cudaMemcpyHostToDevice, streams[s]);

        contract_tensor<<<dimGrid.x / nStreams, dimBlock.x, sharedMemSize, streams[s]>>>(state_d + offset, H_d, 0, new_state_d + offset, shape_d, new_idx_d, old_idx_d, n, N, offset);
    ...
    // contract_tensor<<<dimGrid.x, dimBlock.x, sharedMemSize>>>(state_d, H_d, 0, new_state_d, shape_d, new_idx_d, old_idx_d, n, N);
    ...
    updateState<<<dimGrid.x, dimBlock.x>>>(state_d, new_state_d, N);
