...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < num_groups/2; ++i) {
        ...
        for (int h = 0; h < num_devices; ++h) {
            ...
            initStateParallel<<<dimGrid, dimBlock2, 0, streams[device_id]>>>(state_d[device_id], N_group, N_chunk);
            ...
            for (int l = 0; l < k; ++l) {
                if (index == oracle_group) {
                    ...
                    applyPhaseFlip<<<dimGrid, dimBlock2, 0, streams[device_id]>>>(state_d[device_id], markedState);
                }
