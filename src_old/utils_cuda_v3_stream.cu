...
__global__ void contract_tensor(
    ...
    if ((idx >= lower) & (idx < upper)) {
        ...
        for (int j = 0; j < 2; ++j) {
            ...
            shared_mem[2*idx+j] = cuCmul(gate[new_idx[offset + qubit] * 2 + j], state[old_linear_idx]);
            // printf("value[%d]: %.15f\n", idx, cuCreal(shared_mem[2*idx+j]));
        }
        ...
        state[idx % chunk_size] = cuCadd(shared_mem[2*idx + 1], shared_mem[2*idx]);
