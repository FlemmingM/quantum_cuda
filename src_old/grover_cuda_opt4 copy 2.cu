...
int main(int argc, char* argv[]) {
    ...
    long long int num_groups = (long long int)pow(2, ((n - 20 < 0) ? 0 : (n - 20)));
    printf("num_groups: %d\n", num_groups);
    ...
    int N_group = (int)(pow(2, qubits_per_chunk));
    ...
    if (N_group > 1024) {
        block_factor = N_group / 1024;
        block_size = 1024;
        grid_size = num_chunks_per_group * block_factor;
    } else {
        block_size = N_group;
        grid_size = num_chunks_per_group;
    }
